...
def consensus():
    ...
    for blockchain in blockchains: #전체 노드의 블록 순회
        if len(blockchain['chain']) > max_chain_length: # 현재 노드의 블록보다 더 큰 노드를 찾았을 때
            max_chain_length=len(blockchain['chain'])#max_chain_length에 찾은 노드의 체인 길이 저장
            new_longest_chain=blockchain['chain']#가장 긴 노드를 현재 노드의 체인에서 새로운 노드의 체인으로 변경
